...
export default function SlaCalendarsPage() {
  ...
  const columns = [
    ...
    { field: "include_saturday", headerName: "Sat", width: 80, renderCell: ({ value }) => value ? "Included" : "Excluded" },
    { field: "include_sunday", headerName: "Sun", width: 80, renderCell: ({ value }) => value ? "Included" : "Excluded" },
    { field: "is_active", headerName: "Status", width: 110, renderCell: ({ value }) => <Chip size="small" label={value ? "Active" : "Inactive"} color={value ? "success" : "default"} variant="outlined" /> },
  ];
    ...
    <Stack spacing={2.5} flex={1} minHeight={0}>
      <PageHeader
        ...
        actions={
          <CanAccess permission={SLA_PERMISSIONS.calendarCreate}>
            <Button component={Link} to={`${SLA_ROUTES.calendars}/new`} variant="contained" startIcon={<AddOutlinedIcon />}>Create calendar</Button>
          </CanAccess>
        }
      />
      <TextField size="small" label="Search calendars" value={search} onChange={(event) => { setSearch(event.target.value); setPageModel((current) => ({ ...current, page: 0 })); }} sx={{ maxWidth: 360 }} />
      {error ? <Alert severity="error">{error.response?.data?.message ?? error.message ?? "Unable to load calendars."}</Alert> : null}
      <Paper variant="outlined" sx={{ flex: 1, minHeight: 420 }}>
        <DataGrid
          ...
          onRowClick={({ id }) => navigate(SLA_ROUTES.calendarDetail(id))}
          sx={{ border: 0 }}
        />
      </Paper>
    </Stack>
