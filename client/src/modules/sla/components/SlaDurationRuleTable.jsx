                ...
                <TableCell>
                  <TextField
                    ...
                    inputProps={{
                      min: 1,
                      step: 1,
                      "aria-label": `${row.label} resolution minutes`,
                    }}
                    value={row.resolutionMinutes ?? ""}
                    placeholder="No SLA"
                    onChange={(event) =>
                      update(row.fieldValueKey, {
                        resolutionMinutes:
                          event.target.value === ""
                            ? null
                    ...
                    helperText={
                      row.resolutionMinutes
                        ? formatDurationMinutes(row.resolutionMinutes)
                        : "Not tracked"
                    }
                  />
                </TableCell>
                <TableCell>
                  <Stack direction="row" alignItems="center">
                    <Checkbox
                      ...
                      inputProps={{
                        "aria-label": `Enable ${row.label} SLA rule`,
                      }}
                    />
                  ...
                  <Tooltip title="Remove rule">
                    <IconButton
                      ...
                      onClick={() =>
                        onChange(
                          rows.filter(
                            (item) => item.fieldValueKey !== row.fieldValueKey,
                          ),
                        )
                      }
                    >
