...
export default function UserTable({ users, loading, currentRoles, onEdit, onStatusChange, onDelete, canManageUser }) {
  const columns = [
    ...USER_COLUMNS.map((column) =>
      column.field === "role"
        ? {
            ...column,
            valueGetter: (_value, row) => row.role?.name ?? row.role?.code ?? "—",
          }
        : column,
    ),
    {
  ...
  return (
    <DataGrid
      ...
      getRowId={(row) => row.id}
      slots={{ noRowsOverlay: () => <Box sx={{ p: 4 }}>No users found.</Box> }}
    />
  );
}
