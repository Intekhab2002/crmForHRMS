import{randomUUID as I}from"node:crypto";import q from"dotenv";import W from"node:path";q.config({path:W.resolve(process.cwd(),".env")});var G=["NODE_ENV","PORT","DB_HOST","DB_PORT","DB_NAME","DB_USER","DB_PASSWORD","JWT_SECRET","COOKIE_SECRET","CORS_ALLOWED_ORIGINS","HOST"],N=Object.freeze(["development","production","test"]),_=(e,t)=>{let r=Number(e);if(Number.isNaN(r))throw new Error(`${t} must be a valid number.`);return r},R=(e,t,r)=>e===void 0||e===""?r:_(e,t),X=e=>{e.forEach(t=>{if(!process.env[t])throw new Error(`Missing required environment variable: ${t}`)})};X(G);if(!N.includes(process.env.NODE_ENV))throw new Error(`Invalid NODE_ENV: ${process.env.NODE_ENV}. Allowed values: ${N.join(", ")}`);var Q=Object.freeze({app:Object.freeze({name:process.env.APP_NAME,version:process.env.APP_VERSION,environment:process.env.NODE_ENV}),cookies:Object.freeze({secret:process.env.COOKIE_SECRET,secure:process.env.COOKIE_SECURE==="true",sameSite:process.env.COOKIE_SAMESITE}),server:Object.freeze({host:process.env.HOST,port:_(process.env.PORT,"PORT"),trustProxy:process.env.TRUST_PROXY==="true"?!0:process.env.TRUST_PROXY==="false"?!1:Number.isInteger(Number(process.env.TRUST_PROXY))?Number(process.env.TRUST_PROXY):process.env.TRUST_PROXY}),database:Object.freeze({host:process.env.DB_HOST,port:_(process.env.DB_PORT,"DB_PORT"),database:process.env.DB_NAME,user:process.env.DB_USER,password:process.env.DB_PASSWORD,maxConnections:R(process.env.DB_MAX_CONNECTIONS,"DB_MAX_CONNECTIONS",20),idleTimeout:R(process.env.DB_IDLE_TIMEOUT,"DB_IDLE_TIMEOUT",3e4),connectionTimeout:R(process.env.DB_CONNECTION_TIMEOUT,"DB_CONNECTION_TIMEOUT",3e4),dbSlowQueryThreshold:R(process.env.DB_SLOW_QUERY_THRESHOLD,"DB_SLOW_QUERY_THRESHOLD",2e3),connectionRetries:R(process.env.DB_CONNECTION_RETRIES,"DB_CONNECTION_RETRIES",5),connectionRetryDelay:R(process.env.DB_CONNECTION_RETRY_DELAY,"DB_CONNECTION_RETRY_DELAY",1e3)}),jwt:Object.freeze({secret:process.env.JWT_SECRET,expiresIn:process.env.JWT_EXPIRES_IN,refreshExpiresIn:process.env.JWT_REFRESH_EXPIRES_IN}),logging:Object.freeze({level:process.env.LOG_LEVEL,maxSize:process.env.LOG_MAX_SIZE,maxFiles:process.env.LOG_MAX_FILES,zippedArchive:process.env.LOG_ZIPPED_ARCHIVE==="true",logDirectory:process.env.LOG_DIRECTORY}),security:Object.freeze({bcryptSaltRounds:_(process.env.BCRYPT_SALT_ROUNDS,"BCRYPT_SALT_ROUNDS"),rateLimitWindow:_(process.env.RATE_LIMIT_WINDOW,"RATE_LIMIT_WINDOW"),rateLimitMaxRequests:_(process.env.RATE_LIMIT_MAX_REQUESTS,"RATE_LIMIT_MAX_REQUESTS"),rateLimitAuthWindow:_(process.env.RATE_LIMIT_AUTH_WINDOW,"RATE_LIMIT_AUTH_WINDOW"),rateLimitAuthMaxRequests:_(process.env.RATE_LIMIT_AUTH_MAX_REQUESTS,"RATE_LIMIT_AUTH_MAX_REQUESTS")}),compression:Object.freeze({threshold:process.env.COMPRESSION_THRESHOLD??"1kb"}),features:Object.freeze({swagger:process.env.ENABLE_SWAGGER==="true",requestLogging:process.env.ENABLE_REQUEST_LOGGING==="true"}),http:Object.freeze({jsonLimit:process.env.JSON_LIMIT??"1mb",urlEncodedLimit:process.env.URLENCODED_LIMIT??"1mb",corsAllowedOrigins:process.env.CORS_ALLOWED_ORIGINS.split(",").map(e=>e.trim()).filter(Boolean),corsCredentials:process.env.CORS_CREDENTIALS==="true",parameterLimit:_(process.env.PARAMETER_LIMIT,"PARAMETER_LIMIT"),apiPrefix:process.env.API_PREFIX,apiVersion:process.env.API_VERSION}),seeding:Object.freeze({developerUsername:process.env.SEED_DEVELOPER_USERNAME,developerEmail:process.env.SEED_DEVELOPER_EMAIL,developerPassword:process.env.SEED_DEVELOPER_PASSWORD}),ssl:Object.freeze({enabled:process.env.DB_SSL==="true",rejectUnauthorized:process.env.DB_SSL_REJECT_UNAUTHORIZED==="true",ca:process.env.DB_SSL_CA?.trim()||null})}),m=Q;var K=Object.freeze({app:m.app,server:m.server,database:m.database,jwt:m.jwt,logging:m.logging,http:m.http,cookies:m.cookies,compression:m.compression,security:m.security,features:m.features,seeding:m.seeding,ssl:m.ssl}),o=K;import{Pool as te}from"pg";import{performance as b}from"node:perf_hooks";import re from"p-retry";import y from"node:fs";import T from"node:path";import u from"winston";import g from"winston-daily-rotate-file";var f=T.resolve(o.logging.logDirectory||T.join(process.cwd(),"logs")),J=["combined","error","http","exceptions","rejections"];y.existsSync(f)||y.mkdirSync(f);J.forEach(e=>{let t=T.join(f,e);y.existsSync(t)||y.mkdirSync(t,{recursive:!0})});var v={levels:{error:0,warn:1,info:2,http:3,verbose:4,debug:5},colors:{error:"red",warn:"yellow",info:"green",http:"magenta",verbose:"cyan",debug:"blue"}};u.addColors(v.colors);var D=u.format.combine(u.format.timestamp(),u.format.errors({stack:!0}),u.format.metadata({fillExcept:["message","level","timestamp","label"]}),u.format.json()),Z=u.format.combine(u.format.colorize(),u.format.timestamp({format:"YYYY-MM-DD HH:mm:ss"}),u.format.printf(({timestamp:e,level:t,message:r,stack:s})=>`${e} ${t}: ${s||r}`)),ee=[new u.transports.Console({format:Z}),new g({dirname:T.join(f,"combined"),filename:"%DATE%.log",datePattern:"YYYY-MM-DD",maxSize:o.logging.maxSize,maxFiles:o.logging.maxFiles,zippedArchive:o.logging.zippedArchive==="true",level:o.logging.level,format:D}),new g({dirname:T.join(f,"error"),filename:"%DATE%.log",datePattern:"YYYY-MM-DD",level:"error",maxSize:o.logging.maxSize,maxFiles:o.logging.maxFiles,zippedArchive:o.logging.zippedArchive==="true",format:D}),new g({dirname:T.join(f,"http"),filename:"%DATE%.log",datePattern:"YYYY-MM-DD",level:"http",maxSize:o.logging.maxSize,maxFiles:o.logging.maxFiles,zippedArchive:o.logging.zippedArchive==="true",format:D})],A=u.createLogger({levels:v.levels,level:o.logging.level,transports:ee,exitOnError:!1,exceptionHandlers:[new g({dirname:T.join(f,"exceptions"),filename:"%DATE%.log",datePattern:"YYYY-MM-DD",format:D})],rejectionHandlers:[new g({dirname:T.join(f,"rejections"),filename:"%DATE%.log",datePattern:"YYYY-MM-DD",format:D})]});A.stream={write(e){A.http(e.trim())}};var a=A;var oe={host:o.database.host,port:o.database.port,database:o.database.database,user:o.database.user,password:o.database.password,max:o.database.maxConnections,idleTimeoutMillis:o.database.idleTimeout,connectionTimeoutMillis:o.database.connectionTimeout,allowExitOnIdle:!1,keepAlive:!0,application_name:o.app.name,slowQueryThreshold:o.database.dbSlowQueryThreshold,ssl:o.ssl.enabled?{rejectUnauthorized:o.ssl.rejectUnauthorized,...o.ssl.ca?{ca:o.ssl.ca}:{}}:!1},p=new te(oe);Object.defineProperty(p,"name",{value:"CRM PostgreSQL Pool",writable:!1});p.on("connect",e=>{a.info("New PostgreSQL client connected.",{processId:e.processID})});p.on("acquire",()=>{a.debug("Database client acquired from pool.")});p.on("remove",()=>{a.debug("Database client removed from pool.")});p.on("error",e=>{a.error("Unexpected PostgreSQL pool error.",{error:e.message,stack:e.stack})});async function se(){let e=b.now();try{return await re(async()=>{let t=await p.connect();try{let r=await t.query(`
        ...
        `,[t.code]);if(r.rowCount===0)return(await e.query(`
                ...
                RETURNING
                    ...
                    is_immutable,
                    is_active
            `,[I(),t.code,t.name,t.description])).rows[0];let s=r.rows[0];if(s.is_system!==!0||s.is_immutable!==!0||s.is_active!==!0)throw new Error(`Protected role '${t.code}' exists in an invalid state.`);if(s.name!==t.name)throw new Error(`Protected role '${t.code}' has an invalid name '${s.name}'.`);return s}async function xe(e,t){return(await e.query(`
            ...
            RETURNING
                ...
                is_system,
                is_active
        `,[I(),t.code,t.name,t.description,t.resource,t.action])).rows[0]}async function B(e,t,r){await e.query(`
        ...
        `,[t.username,t.email]);if(r.rowCount>0){let i=r.rows[0];if(i.username.toLowerCase()!==t.username.toLowerCase()||i.email.toLowerCase()!==t.email.toLowerCase())throw new Error("Developer bootstrap username/email conflicts with an existing user.");if(i.status!==$.ACTIVE)throw new Error("Existing Developer bootstrap user is not active.");return i}let s=await x.hashPassword(t.password);return(await e.query(`
            ...
            RETURNING
                ...
                password_changed_at,
                deactivated_at
        `,[I(),t.username,t.email,s,$.ACTIVE])).rows[0]}async function $e(e,t,r){await e.query(`
        ...
        `,[t,r])}async function Be(e,t,r,s){if((await e.query(`
            ...
            `)).rows[0].count!==2)throw new Error("Developer and Super Admin system-role validation failed.");a.info("RBAC bootstrap state validated successfully.",{developerUserId:t.id,developerRoleId:r.id,superadminRoleId:s.id})}async function Fe(){Pe(),Me();let e=je();await O.initialize();try{await C(async t=>{let r=M(t);await r.query("SET LOCAL app.rbac_bootstrap = 'true'");let s=new Map;for(let l of w){let c=await He(r,l);s.set(c.code,c)}let n=s.get("developer"),i=s.get("superadmin");if(!n||!i)throw new Error("Required system roles are unavailable.");let E=new Map;for(let l of S){let c=await xe(r,l);E.set(c.code,c)}for(let l of Y){let c=E.get(l);if(!c)throw new Error(`Developer permission was not found: ${l}`);await B(r,n.id,c.id)}for(let l of F){let c=E.get(l);if(!c)throw new Error(`Super Admin permission was not found: ${l}`);await B(r,i.id,c.id)}let d=await Ve(r,e);await $e(r,d.id,n.id),await Be(r,d,n,i),a.info("Initial RBAC bootstrap completed.",{roleCount:w.length,permissionCount:S.length,developerUser:d.email,superadminUserCount:0})})}finally{await O.close()}}try{await Fe(),a.info("RBAC bootstrap seeder completed successfully.")}catch(e){a.error("RBAC bootstrap seeder failed.",{message:e.message,stack:e.stack}),process.exitCode=1}
