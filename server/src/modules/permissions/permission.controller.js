...
import { PERMISSION_MESSAGES } from "./permission.constant.js";

async function getPermissions(request, response, next) {
  try {
...
async function getPermissionById(request, response, next) {
  try {
...
async function createPermission(request, response, next) {
  try {
...
async function updatePermission(request, response, next) {
  try {
    const permission = await permissionService.updatePermission(
      request.validatedParams.permissionId,
      request.validatedBody,
      request.auth.userId,
    );
...
async function deletePermission(request, response, next) {
  try {
