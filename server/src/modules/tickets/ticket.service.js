...
const CONTACT_INPUT_KEYS = Object.freeze([
    "name",
    "contact_name",
    "mobile_phone",
    "email_id",
...
async function validateReferences(data, tx) {
    ...
    if (data.contact) {
        const contact = await ticketRepository.findContact(
            data.contact,
            tx,
        );
...
async function updateTicket(ticketId, payload) {
    ...
    return executeTransaction(async (tx) => {
        ...
        const effective = {
            department:
                ticket.department ?? current.department_id,
            organization:
                ticket.organization ?? current.organization_id,
            requester_user_id:
                ticket.requester_user_id ?? current.requester_user_id,
            assigned_to:
                Object.prototype.hasOwnProperty.call(ticket, "assigned_to")
                    ? ticket.assigned_to
                    : current.assigned_user_id,
            contact:
                Object.prototype.hasOwnProperty.call(ticket, "contact")
                    ? ticket.contact
                    : current.contact_id,
        };
