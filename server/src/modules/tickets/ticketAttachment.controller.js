...
async function listAttachments(req, res, next) {
  try {
    ...
    const attachments = await ticketAttachmentService.listAttachments(ticketId);
...
async function viewAttachment(req, res, next) {
  try {
    const { ticketId, attachmentId } = req.params;

    const result =
      await ticketAttachmentService.getAttachmentFile(attachmentId);
    ...
    const filename = sanitizeDownloadFilename(attachment.original_name);
...
async function deleteAttachment(req, res, next) {
  try {
    ...
    const attachment =
      await ticketAttachmentService.getAttachment(attachmentId);

    if (!attachment || attachment.ticket_id !== ticketId) {
      throw AppError.notFound("Attachment not found.");
    }

    const deleted = await ticketAttachmentService.deleteAttachment(
      attachmentId,
      req.auth.userId,
    );
