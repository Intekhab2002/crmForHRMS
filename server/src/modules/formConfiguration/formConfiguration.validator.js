...
const optionsConfigSchema = z
  .object({
    static: z.array(staticOptionSchema).max(500).optional(),
    dataSource: z
      .object({
...
const storageTypeSchema = z.enum(STORAGE_TYPES);

const storageMappingSchema = z
  .object({
    storageType: storageTypeSchema.optional(),
    storageColumn: z.string().trim().max(150).nullable().optional(),
    storageKey: z.string().trim().max(150).nullable().optional(),
    referenceEntity: z.string().trim().max(100).nullable().optional(),
  })
  .strict();
...
const createFieldSchema = z
  .object({
    ...
    validationConfig: validationConfigSchema.default({}),
    optionsConfig: optionsConfigSchema.default({}),
    ...storageMappingSchema.shape,
  })
  .strict();
...
const fieldIdParamSchema = z.object({ fieldId: uuidSchema }).strict();
...
const formCodeParamSchema = z.object({ formCode: formCodeSchema }).strict();
...
const formIdParamSchema = z.object({ formId: uuidSchema }).strict();
...
const assignFieldSchema = z
  .object({
    ...
    isSearchable: z.boolean().nullable().optional(),
    ...
    gridSize: z.number().int().min(1).max(12).nullable().optional(),
    columnWidth: z.string().trim().max(50).nullable().optional(),
    labelOverride: z.string().trim().max(200).nullable().optional(),
    placeholderOverride: z.string().trim().max(255).nullable().optional(),
    helpTextOverride: z.string().trim().max(2000).nullable().optional(),
    defaultValueOverride: z.unknown().optional(),
  })
  .strict();
