...
import { assertSafeFieldStructuralEdit } from "./fieldAdministrationSafety.js";

function assertFieldId(fieldId) {
  if (typeof fieldId !== "string" || fieldId.trim().length === 0) {
    throw AppError.badRequest("Field ID is required.", {
...
function normalizeConflict(error) {
  if (error?.code === "23505") {
    throw AppError.conflict("A field with this key already exists.", {
...
async function getFieldById(fieldId) {
  ...
  if (!field) {
    throw AppError.notFound("Form field not found.", {
...
async function updateField(fieldId, data, actorId) {
  ...
  assertSafeFieldStructuralEdit(existing, data, usage);

  validateFieldConfiguration(data, existing);

  try {
    return await metadataRepository.updateField(fieldId, data, actorId);
  } catch (error) {
    return normalizeConflict(error);
  }
}
...
async function deleteField(fieldId, actorId) {
  ...
  if (existing.is_deleted) {
    throw AppError.conflict("The form field is already deleted.", {
...
async function restoreField(fieldId, actorId) {
  ...
  if (!existing.is_deleted) {
    throw AppError.conflict("The form field is not deleted.", {
...
async function createForm(data, actorId) {
  ...
  if (existing) {
    throw AppError.conflict("A form with this code already exists.", {
...
async function assignField(formId, fieldId, data, actorId) {
  ...
  if (existing) {
    throw AppError.conflict("This field is already assigned to the form.", {
      code: ASSIGNMENT_ERROR_CODES.ALREADY_ASSIGNED,
    });
  }

  try {
    await repository.createAssignment({ formId, fieldId, ...data }, actorId);
...
async function removeField(formId, fieldId) {
  ...
  if (!assignment) {
    throw AppError.notFound("Field assignment not found.", {
...
async function updateAssignment(formId, fieldId, data, actorId) {
  const form = await repository.findFormById(formId);
  ...
  const requestedDisplayOrder = data.displayOrder;

  if (requestedDisplayOrder !== undefined && requestedDisplayOrder !== null) {
    const assignments = await repository.findAssignmentsByFormId(formId);

    const conflictingAssignment = assignments.find(
      (item) =>
        item.fieldId !== fieldId && item.displayOrder === requestedDisplayOrder,
    );

    if (conflictingAssignment) {
      throw AppError.conflict(
        "Display order is already assigned to another field.",
        {
          code: "FORM_FIELD_DISPLAY_ORDER_CONFLICT",
          meta: {
            formId,
            fieldId,
  ...
  await repository.updateAssignment(formId, fieldId, data, actorId);
