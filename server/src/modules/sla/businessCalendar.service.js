...
function holidaysSet(holidays = []) {
  return new Set(
    holidays
      .map((h) =>
        typeof h === "string" ? h : (h.holiday_date ?? h.holidayDate),
      )
      .filter(Boolean),
  );
}
...
export function getBusinessWindow(value, calendar) {
  ...
  const [sh, sm] = String(calendar.workday_start_time)
    .slice(0, 5)
    .split(":")
    .map(Number);

  const [eh, em] = String(calendar.workday_end_time)
    .slice(0, 5)
    .split(":")
    .map(Number);

  const start = valueDt.startOf("day").set({
    hour: sh,
    minute: sm,
    second: 0,
    millisecond: 0,
  ...
  const end = valueDt.startOf("day").set({
    hour: eh,
    minute: em,
    second: 0,
    millisecond: 0,
  ...
  if (end <= start) {
    throw new TypeError(
      "workday_end_time must be later than workday_start_time.",
    );
  }
