import { addBusinessMinutes, calculateBusinessMinutes } from "./businessTime.service.js";
...
export async function syncTicket(ticketId, { now = new Date(), tx = null } = {}) {
    const ticket = await repository.oneTicket(ticketId, tx);
    if (!ticket) return null;

    const runtime = await repository.oneTicketSla(ticketId, tx);

    if (runtime && [SLA_STATUS.STOPPED, SLA_STATUS.COMPLETED, SLA_STATUS.BREACHED].includes(runtime.status)) {
        return runtime;
    }
    ...
    if (runtime && activePolicy) {
        ...
        if (!rule || !rule.is_enabled || rule.resolution_minutes === null) {
            ...
            return repository.updateTicketSla(runtime.id, {
    ...
    if (!rule || !rule.is_enabled || rule.resolution_minutes === null) {
        return ticketSla.setNotTracked(ticket, resolvedPolicy, tx);
    }
