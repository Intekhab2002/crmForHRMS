...
async function listPolicies(
  { page = 1, limit = 20, search = null, isActive = null } = {},
  tx = null,
) {
  const where = [],
    params = [];
  ...
  const count = await ex(tx).query(
    `SELECT COUNT(*)::bigint total FROM sla_policies p ${clause}`,
    params,
  );
  params.push(limit, (page - 1) * limit);
  const rows = await ex(tx).query(
    `${POLICY_SELECT} ${clause} ORDER BY p.priority DESC,p.effective_from DESC,p.name LIMIT $${i} OFFSET $${i + 1}`,
    params,
  );
...
async function applicablePolicy(field, value, at, tx = null) {
 ...
 ORDER BY p.priority DESC,p.effective_from DESC,p.created_at DESC LIMIT 1`,
    [field, value, at],
  );
  return r.rows[0] ?? null;
}
...
async function updatePolicy(id, d, tx = null) {
 ...
 updated_by_user_id=$15 WHERE id=$1 RETURNING id`,
    [
      ...
      d.code ?? null,
      d.name ?? null,
      Object.hasOwn(d, "description"),
      d.description ?? null,
      d.isActive ?? null,
      d.triggerFieldKey ?? null,
      d.triggerValueKey ?? null,
      d.durationFieldKey ?? null,
      d.calendarId ?? null,
      d.priority ?? null,
      d.effectiveFrom ?? null,
      Object.hasOwn(d, "effectiveTo"),
      d.effectiveTo ?? null,
      d.actorUserId ?? null,
    ],
  );
...
async function setPolicyActive(id, active, actor, tx = null) {
  const r = await ex(tx).query(
    `UPDATE sla_policies SET is_active=$2,updated_by_user_id=$3 WHERE id=$1 RETURNING id`,
    [id, active, actor ?? null],
  );
  return r.rows[0] ? onePolicy(id, tx) : null;
}
async function oneCalendar(id, tx = null) {
  const r = await ex(tx).query(`${CALENDAR_SELECT} WHERE id=$1 LIMIT 1`, [id]);
  return r.rows[0] ?? null;
}
async function listCalendars(
  ...
  const clause = where.length ? `WHERE ${where.join(" AND ")}` : "";
  const count = await ex(tx).query(
    `SELECT COUNT(*)::bigint total FROM sla_calendars ${clause}`,
    params,
  );
  params.push(limit, (page - 1) * limit);
  const rows = await ex(tx).query(
    `${CALENDAR_SELECT} ${clause} ORDER BY name LIMIT $${i} OFFSET $${i + 1}`,
    params,
  );
  return { rows: rows.rows, total: Number(count.rows[0]?.total ?? 0) };
}
async function createCalendar(d, tx = null) {
  const r = await ex(tx).query(
    `INSERT INTO sla_calendars(code,name,timezone,business_hours_per_day,workday_start_time,workday_end_time,include_saturday,include_sunday,is_active)
 VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,
    [
      d.code,
      d.name,
      d.timezone,
      d.businessHoursPerDay,
      d.workdayStartTime,
      d.workdayEndTime,
      d.includeSaturday,
      d.includeSunday,
...
async function updateCalendar(id, d, tx = null) {
 ...
 is_active=COALESCE($10,is_active) WHERE id=$1 RETURNING id`,
    [
      ...
      d.name ?? null,
      d.timezone ?? null,
      d.businessHoursPerDay ?? null,
      d.workdayStartTime ?? null,
      d.workdayEndTime ?? null,
      d.includeSaturday ?? null,
      d.includeSunday ?? null,
      d.isActive ?? null,
    ],
  );
...
async function setCalendarActive(id, active, tx = null) {
  const r = await ex(tx).query(
    `UPDATE sla_calendars SET is_active=$2 WHERE id=$1 RETURNING id`,
    [id, active],
  );
  return r.rows[0] ? oneCalendar(id, tx) : null;
}
async function listHolidays(calendarId, { year = null } = {}, tx = null) {
  const params = [calendarId],
    where = ["calendar_id=$1", "is_active=TRUE"];
  if (year) {
...
async function oneHoliday(id, tx = null) {
  const r = await ex(tx).query(
    `SELECT * FROM sla_calendar_holidays WHERE id=$1 LIMIT 1`,
    [id],
  );
  return r.rows[0] ?? null;
}
async function createHoliday(d, tx = null) {
  const r = await ex(tx).query(
    `INSERT INTO sla_calendar_holidays(calendar_id,holiday_date,name,is_active) VALUES($1,$2,$3,$4) RETURNING id`,
    [d.calendarId, d.holidayDate, d.name, d.isActive ?? true],
  );
  return oneHoliday(r.rows[0].id, tx);
}
async function updateHoliday(id, d, tx = null) {
  const r = await ex(tx).query(
    `UPDATE sla_calendar_holidays SET holiday_date=COALESCE($2,holiday_date),name=COALESCE($3,name),is_active=COALESCE($4,is_active) WHERE id=$1 RETURNING id`,
    [id, d.holidayDate ?? null, d.name ?? null, d.isActive ?? null],
  );
...
async function upsertTicketSla(d, tx = null) {
 ...
 ON CONFLICT(ticket_id) DO UPDATE SET sla_policy_id=EXCLUDED.sla_policy_id,status=EXCLUDED.status,activated_at=EXCLUDED.activated_at,paused_at=EXCLUDED.paused_at,stopped_at=EXCLUDED.stopped_at,completed_at=EXCLUDED.completed_at,breached_at=EXCLUDED.breached_at,target_resolution_minutes=EXCLUDED.target_resolution_minutes,elapsed_business_minutes=EXCLUDED.elapsed_business_minutes,remaining_business_minutes=EXCLUDED.remaining_business_minutes,last_calculated_at=EXCLUDED.last_calculated_at,activation_field_key=EXCLUDED.activation_field_key,activation_field_value_key=EXCLUDED.activation_field_value_key,duration_field_key=EXCLUDED.duration_field_key,duration_field_value_key=EXCLUDED.duration_field_value_key,policy_snapshot=EXCLUDED.policy_snapshot RETURNING id`,
    [
      ...
      d.completedAt ?? null,
      d.breachedAt ?? null,
      d.targetResolutionMinutes ?? null,
      d.elapsedBusinessMinutes ?? 0,
      d.remainingBusinessMinutes ?? null,
      d.lastCalculatedAt ?? null,
      d.activationFieldKey ?? null,
      d.activationFieldValueKey ?? null,
      d.durationFieldKey ?? null,
      d.durationFieldValueKey ?? null,
      d.policySnapshot ?? {},
    ],
  );
...
async function updateTicketSla(id, d, tx = null) {
  const map = [
    ...
    ["breached_at", d.breachedAt],
    ["target_resolution_minutes", d.targetResolutionMinutes],
    ["elapsed_business_minutes", d.elapsedBusinessMinutes],
    ["remaining_business_minutes", d.remainingBusinessMinutes],
    ["last_calculated_at", d.lastCalculatedAt],
    ["activation_field_key", d.activationFieldKey],
    ["activation_field_value_key", d.activationFieldValueKey],
    ["duration_field_key", d.durationFieldKey],
    ["duration_field_value_key", d.durationFieldValueKey],
  ...
  for (const [col, v] of map)
    if (v !== undefined) {
      set.push(`${col}=$${i++}`);
      params.push(col === "policy_snapshot" ? JSON.stringify(v) : v);
    }
  if (!set.length) return null;
  const r = await ex(tx).query(
    `UPDATE ticket_sla SET ${set.join(",")} WHERE id=$1 RETURNING ticket_id`,
    params,
  );
...
async function createRule(d, tx = null) {
  const r = await ex(tx).query(
    `INSERT INTO sla_policy_rules(sla_policy_id,field_value_key,resolution_minutes,is_enabled) VALUES($1,$2,$3,$4) RETURNING *`,
    [d.policyId, d.fieldValueKey, d.resolutionMinutes, d.isEnabled ?? true],
  );
  return r.rows[0];
}
async function updateRule(id, d, tx = null) {
  const r = await ex(tx).query(
    `UPDATE sla_policy_rules SET field_value_key=COALESCE($2,field_value_key),resolution_minutes=CASE WHEN $3::boolean THEN $4 ELSE resolution_minutes END,is_enabled=COALESCE($5,is_enabled) WHERE id=$1 RETURNING *`,
    [
      id,
      d.fieldValueKey ?? null,
      Object.hasOwn(d, "resolutionMinutes"),
      d.resolutionMinutes ?? null,
      d.isEnabled ?? null,
    ],
  );
  return r.rows[0] ?? null;
