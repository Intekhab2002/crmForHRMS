...
function assertField(key){
    if(!config.supportedFields.includes(key)){
        throw new AppError({message:`Unsupported SLA field: ${key}`,statusCode:422,code:SLA_ERROR_CODES.FIELD_NOT_SUPPORTED});
    }
}
async function requirePolicy(id,tx=null){
    const item=await repository.onePolicy(id,tx);
    if(!item) throw AppError.notFound("SLA policy not found.",{code:SLA_ERROR_CODES.POLICY_NOT_FOUND});
    return item;
}
async function list(options={}){return repository.listPolicies(options);}
async function getById(id){return requirePolicy(id);}
async function create(data,actorUserId,tx=null){
    assertField(data.triggerFieldKey); assertField(data.durationFieldKey);
    return repository.createPolicy({...data,actorUserId,code:data.code.toLowerCase(),triggerValueKey:data.triggerValueKey.toLowerCase() },tx);
}
async function update(id,data,actorUserId,tx=null){
    ...
    if(normalized.triggerValueKey)normalized.triggerValueKey=normalized.triggerValueKey.toLowerCase();
...
async function resolve(field,value,at,tx=null){
    assertField(field);
    return repository.applicablePolicy(field,value,at,tx);
}
export default Object.freeze({list,getById,requirePolicy,create,update,setActive,resolve});
