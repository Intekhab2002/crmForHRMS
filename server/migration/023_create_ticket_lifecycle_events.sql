...
CREATE INDEX IF NOT EXISTS ticket_lifecycle_events_ticket_idx
    ON ticket_lifecycle_events (
        ticket_id,
        created_at DESC
    );

CREATE INDEX IF NOT EXISTS ticket_lifecycle_events_actor_idx
    ON ticket_lifecycle_events (
        actor_user_id
    );

CREATE INDEX IF NOT EXISTS ticket_lifecycle_events_type_idx
    ON ticket_lifecycle_events (
        event_type,
        event_action
    );

CREATE INDEX IF NOT EXISTS ticket_lifecycle_events_created_at_idx
    ON ticket_lifecycle_events (
        created_at DESC
    );